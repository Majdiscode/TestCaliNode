...
// MARK: - Mini Quest Widget (for other views)

struct MiniQuestWidget: View {
    @ObservedObject var questManager: QuestManager
    let maxDisplayed: Int = 2
    ...
    var body: some View {
        if !questManager.activeQuests.isEmpty {
            VStack(alignment: .leading, spacing: 12) {
                ...
                VStack(spacing: 8) {
                    ForEach(Array(questManager.activeQuests.prefix(maxDisplayed))) { quest in
                        MiniQuestRow(quest: quest)
                    }
                    
                    if questManager.activeQuests.count > maxDisplayed {
                        Text("+ \(questManager.activeQuests.count - maxDisplayed) more")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                }
    ...
    var body: some View {
        ZStack {
            ...
            Circle()
                ...
                .stroke(progress >= 1.0 ? .green : .blue, lineWidth: 2)
                .frame(width: size, height: size)
